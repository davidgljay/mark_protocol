...
1. Contents of a Chitt Template Policy
A Chitt template policy is a structured JSON document, signed by the authorizing Chitt, that fully specifies how a Chitt Press will operate. It contains:

Identity and authority:

A stable policy ID (CID of the signed policy blob)
Mutable pointer of the authorizing Chitt (e.g., the superintendent's Chitt)
Mutable pointers of any audit-authority Chitts (often but not necessarily the same as the authorizer)
valid_until timestamp bounding the template's authorized lifetime

Output Chitt specification:

Schema for the Chitts this template will produce (field names, types, required vs. optional)
A schema reference that issued Chitts will embed, allowing verifiers to confirm conformance
Recipient mode: targeted (issuer specifies recipient), open (anyone presenting valid inputs), or requested (recipient initiates)
An optional image CID field: a content-addressed IPFS link to an image representing the Chitt visually, intended for display in a hexagonal frame. If the policy includes this field, the issuer may supply a specific image CID at issuance time; if left unspecified, no image is included in the issued Chitt

Authorization rules (who can invoke):

A predicate over the requester's Chitt chain (e.g., "Chitt must chain to superintendent Chitt X via the school-administrator template")
...
Required co-signed statements from specified Chitt types
...
Log mode: exclusivity (one update per Chitt for verifiable counting), timing-cover (constant-rate updates to hide issuance timing), or minimal
Log entry granularity (per-Chitt or batched-with-public-count)
Revocation freshness window (max staleness for revocation status checks during issuance)
Notification destinations for issuance summaries (typically the authorizer's Chitt)

Annotation policy (optional):

A reference to a signed annotation policy governing who may publish issuer annotations to Chitts produced by this template, and what kinds of entries they may add
If absent, only the original issuer may annotate

Revocation semantics:

Whether revocation of the template Chitt cascades to Chitts it issued, and under what reason codes (policy revocation: forward-only; key compromise: retroactive)
...
The authorizer reviews the policy. If approved, she signs the policy blob with her Chitt's private key.
...
The policy ID is the durable identifier that survives across Chitt Press deployments. Press instances come and go (with hoster changes, code upgrades, key rotations); the policy ID remains stable, and Chitts issued by successive deployments of the same policy form a coherent set.

3. Shopping for and Approving an Execution Environment
The administrator selects a Chitt Press hosting service. Chitt Presses are operated as a service and can accept arbitrary approved policies. The protocol's trust model means the hoster cannot forge Chitts — they can only run or refuse to run the attested code — so the choice is about reliability, jurisdiction, attestation transparency, and price rather than about extending trust to the hoster's intentions.

The deployment and approval flow:

The administrator sends the signed policy blob to the hoster's deployment endpoint.
The hoster spins up an enclave (e.g., Nitro Enclave on a parent EC2 instance) running the standard, audited, open-source Chitt Press code. In multi-tenant configurations, an existing shared enclave may add the new policy to its loaded set rather than spinning up dedicated infrastructure.
...
The superintendent signs a template Chitt binding her authority to:
...
The administrator (or the enclave on the administrator's behalf) assembles a proposed Chitt JSON per the policy schema, with all issuer-populated metadata fields filled in. This includes the image CID field if the policy specifies one — the issuer supplies the IPFS CID of the image at this stage, before the enclave signs the offer. The recipient's public key field is left empty — it will be supplied by the recipient.
The enclave signs this proposed Chitt JSON (including the empty public key field) with its keypair, producing a signed offer.
...
Once a participant holds a Chitt, subsequent Chitts can be offered directly via Nym. The enclave sends the signed offer (with empty public key field) to the recipient's existing Chitt's Nym gateway. The recipient evaluates the offer in their client, generates a fresh keypair if accepting (or reuses their master key — an implementation choice), adds their public key and countersignature, and posts the completed Chitt to IPFS.

The mutual-signing pattern: the enclave commits to the Chitt's content (including all issuer-defined fields) by signing the offer. The recipient accepts scope by adding their public key and countersigning. Neither party can unilaterally alter the completed Chitt.

7. Logging, Posting, and Notification
After the recipient posts the completed Chitt to IPFS:

The recipient (or their client) notifies the enclave's endpoint with the CID of the posted Chitt.
...
The enclave produces a Signed Chitt Inclusion Proof (SCIP): a small signed object binding the Chitt's CID to its log entry index and the log root at time of inclusion.
The enclave sends notifications via Nym:

To the recipient: the SCIP and a confirmation.
To the issuer (administrator): the Chitt's CID, the SCIP, and an audit record, encrypted to the administrator's Chitt.
