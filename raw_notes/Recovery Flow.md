...
2. User registers with one or more backup services, presenting their YubiKey. The backup service stores an encrypted blob containing the keyring decryption key, wrapped under the YubiKey-derived key. The backup service returns a Chitt — a signed credential proving this registration exists — and records the user's notification channels and cancellation credentials.

3. Primary service goes down. User goes to a backup service with their YubiKey.

4. Backup service sends notifications to all configured channels simultaneously (Nym to the Chitt's gateway, email, SMS, secondary contacts, whatever was configured at registration). It waits 72 hours for a cancellation signed by any registered cancellation credential.

5. If cancellation is received from any registered credential, the service silently aborts and notifies the Chitt holder with instructions to rotate their backup registration (generate a new YubiKey-wrapped backup, deregister the old one, treat the old YubiKey as potentially compromised).
...
The master private key for each Chitt the holder controls
The private keys for any sub-Chitts registered to those master Chitts
Metadata associating each key with its corresponding Chitt mutable pointer
...
The Chitt as proof of registration: the Chitt returned by the backup service at registration serves double duty — it's a proof of registration that can be verified on-chain, and it provides a Nym gateway address for the 72-hour notification. The recovery system is built on the Chitt protocol's own primitives rather than a separate identity system.

What This Design Does Not Cover
On-chain state: if any Chitt's mutable pointer or append-only log requires on-chain transactions (e.g. revoking a sub-Chitt, posting a new log entry), recovering the keyring restores the signing keys but does not automatically replay any pending on-chain operations. Those must be re-initiated after recovery.

Sub-Chitt deregistration: after recovery, the holder should audit their active sub-Chitts and deregister any that were registered on devices that may have been compromised as part of whatever event triggered the recovery.
