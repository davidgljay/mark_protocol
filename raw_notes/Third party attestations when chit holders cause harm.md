Third-Party Attestations and Resolutions When Chitt Holders Cause Harm

The Problem
The issuer annotation layer assumes annotations come from parties with context — issuers and parties authorized under the annotation policy in the Chitt's issuance chain. This works for credentialing and reputation but fails in two specific cases: when a Chitt holder causes harm and the issuer refuses to act, and when the issuer itself is the source of harm (issuing Chitts under false pretenses, sheltering bad actors). Both cases need annotators who sit outside the chain of trust they are commenting on. This is the role of third-party safety annotators, which operate through the third-party annotation system — distinct from issuer annotations in the Chitt's own append-only log.
...
The fake-doctor case requires controlled downward propagation. A safety annotation on a higher chain link can mark itself as either:

Applies to Chitts issued by this entity (cascading) — concerns the institution and its derivatives. Used for fraudulent credentialing patterns.
Applies only to this specific Chitt (non-cascading) — concerns an individual, not the institution. Used when one school employee misbehaves but the school itself is sound.

Cascading annotations carry an applies_to_chitts_issued_during window to handle remediation — an institution that has cleaned house shouldn't poison new Chitts indefinitely. Without an explicit inheritance marker, verifiers don't know which interpretation to apply, and the annotator is accountable for choosing correctly.
...
Both safety annotators and facilitators require sustainable funding to operate credibly. Trust-and-safety work and restorative justice work are labor-intensive and most existing practitioners are underfunded. Possible models: foundation grants to seed initial capacity, sliding-scale per-process fees, institutional sponsorship by employers and professional associations, protocol-level fees on Chitt issuance routed to a facilitator fund. These are ecosystem questions rather than protocol questions, but the protocol should be friendly to all of them — supporting payment metadata, sponsor relationships, and fee-structure transparency.
...
Coordination with existing T&S organizations. Making it easy for NCMEC, IWF, and similar bodies to participate as annotators — probably through sponsored facilitator/annotator Chitts underwritten by foundations or industry consortia.
...
In a pure adjudicative frame, the safety layer answers "should I trust this Chitt?" — a present-tense judgment. With restorative process annotations, the safety layer also answers "what is the trajectory of trust around this Chitt?" — a temporal question about whether the parties involved are doing the work of being trustworthy over time. Communities that function well in practice don't just sort members into trusted and untrusted bins; they acknowledge harm, support repair, and welcome people back from broken trust without erasing the history. Building that capacity in from the start — rather than retrofitting it after an adjudicative frame has hardened — is the kind of design decision whose value isn't visible until much later.
