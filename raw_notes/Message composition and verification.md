...
For private messages, the entire signed envelope is then encrypted to each recipient's master public key, resolved from the recipient Chitt's current metadata via their mutable pointer. The encrypted envelope is routed through Nym to each recipient's gateway, arrives at the recipient's message server, is transformed via UMBRAL proxy re-encryption into ciphertexts encrypted to each of the recipient's active sub-Chitts, and is queued for device pickup.

Encryption to the recipient Chitt's listed public key prevents network-level interception. It does not prevent Chitt substitution — the attack where you think you're contacting Alice but the pointer you have actually belongs to someone else. That binding between human-meaningful identity and mutable pointer lives outside the cryptographic system and is bootstrapped through out-of-band channels (in-person fingerprint verification, trusted introductions, accumulated annotations).

Verification on Receipt
The recipient's device pulls queued ciphertexts from the message server, decrypts each with the local sub-Chitt private key, and obtains the signed envelope. Verification proceeds in stages:
...
Sub-Chitt to master link. For each valid signature, the client resolves the sub-Chitt's mutable pointer and confirms that this sub-Chitt appears in the active sub-Chitt list of its claimed master Chitt's metadata, and that the master Chitt signed the sub-Chitt registration. This is the device-delegation link.

Master Chitt chain walk. From each master Chitt, the client walks the issuance chain link by link via mutable pointers: fetching each link's current metadata, verifying the issuer's signature, checking the append-only log for revocations, validating that scope at each link does not exceed the issuing Chitt's scope, and continuing up until reaching either a trusted root (success) or an unrecognized root (chain doesn't reach trusted ground).
...
Annotation lookup. The client queries EAS/IPFS for third-party annotations referencing each Chitt in the chain, filters them by whether the annotation signer's own chain validates to a root the recipient trusts, and assembles the relevant annotation context.

Recipient-set check. The client confirms its own Chitt's mutable pointer appears in the message's recipients list. A message whose recipient list does not include the receiving Chitt is technically valid but was not addressed to this recipient — the client may surface this differently (e.g., as a forwarded message rather than a direct one).
...
Authorization to edit. The recipient's client confirms that the edit's signers chain to the same master Chitt(s) as the original's signers. Edits from a different sub-Chitt of the same master are valid (Alice editing from her phone what she sent from her laptop); edits from an unrelated Chitt are not. For co-signed messages, an edit signed by the full original signer set is a full edit; an edit signed by only some original signers is a partial amendment and should be displayed differently.
...
Trust-root configuration UX — how recipients establish which Chitts are trusted roots, and how those settings are managed across devices.
